...
- **Trabalhe SEMPRE na `main`**: nunca crie branch, nunca troque de branch, nunca abra PR como
  entregável. Trocar de branch quebra sessões paralelas que compartilham o mesmo working tree.
...
✅ **Right**: work on `main` → `git diff` → commit incremental → deploy por comando (`railway up`)
❌ **Wrong**: `git checkout -b feature/auth`, `git switch`, entregar PR e parar
**Ponto de restauração** é commit, não branch. Precisa isolar? Commit antes, não branch nova.

**Exceção única**: o usuário pedir branch explicitamente, nesta conversa. Não presuma.
...
- **Agent Delegation**: delegue a subagentes quando o projeto autorizar (ver `CLAUDE.md` §5.4).
  Dentro da esteira IDEAL é o fluxo normal; fora dela, exige pedido. Número de passos não é
  gatilho suficiente por si só.
